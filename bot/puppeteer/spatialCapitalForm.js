...
const fillSpatialCapitalForm = async (formData) => {
    ...
    let browser;
    let page;  // объявляем page здесь, чтобы можно было очистить его позже
    let frame; // объявляем frame здесь, чтобы можно было очистить его позже
    try {
        browser = await puppeteer.launch({ headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox'], executablePath: '/usr/bin/google-chrome-stable' });
